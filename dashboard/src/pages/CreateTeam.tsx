...
import { api, type TeamWizardInput, type Agent, type Specialty } from '../services/api';
...
export default function CreateTeam() {
  ...
  useEffect(() => {
    Promise.all([api.getAgents(), api.getSpecialties(), api.getTemplates()])
      .then(([a, s, t]) => { setAgents(a); setSpecialties(s); setTemplates(t); setLoading(false); })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setStatus('creating');
    try {
      const res = await api.createTeam(input);
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-white mb-1">Team Creation Wizard</h1>
      <p className="text-sm text-gray-500 mb-6">Bundle agents, specialties, and templates into a workforce team for one-command deployment.</p>

      {status === 'done' ? (
        <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-lg p-6">
          <p className="text-emerald-400 font-medium mb-2">Team created successfully</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input required className="w-full bg-white/5 border border-gray-700 rounded px-3 py-2 text-sm text-white placeholder-gray-600 focus:border-accent focus:outline-none" placeholder="What this team provides" value={description} onChange={(e) => setDescription(e.target.value)} />
            </div>
          </div>
          ...
          <div className="bg-white/[0.02] border border-gray-800 rounded-lg p-5">
            <h2 className="text-sm font-semibold text-gray-300 mb-3">
              ...
              <button type="button" onClick={() => setSelectedSpecialties(selectedSpecialties.length === specialties.length ? [] : specialties.map((s) => s.name))} className="ml-3 text-xs text-accent hover:underline">
                {selectedSpecialties.length === specialties.length ? 'Deselect All' : 'Select All'}
              </button>
            </h2>
            <div className="grid grid-cols-3 gap-2">
              {specialties.map((s) => (
                <label key={s.name} className={`flex items-center gap-2 p-2 rounded cursor-pointer border transition-colors ${selectedSpecialties.includes(s.name) ? 'bg-accent/10 border-accent/30' : 'bg-white/[0.02] border-gray-800 hover:border-gray-700'}`}>
                  <input type="checkbox" checked={selectedSpecialties.includes(s.name)} onChange={() => toggleItem(selectedSpecialties, s.name, setSelectedSpecialties)} className="rounded border-gray-600" />
