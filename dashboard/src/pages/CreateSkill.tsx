...
import { api, type SkillWizardInput } from '../services/api';
...
export default function CreateSkill() {
  const [form, setForm] = useState<Partial<SkillWizardInput>>({
    designSystem: false,
    mcpDependencies: [],
    sections: [],
    steps: [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setStatus('creating');
    try {
      const res = await api.createSkill(input);
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-white mb-1">Skill Creation Wizard</h1>
      <p className="text-sm text-gray-500 mb-6">Define a new skill with invocation, output format, and workflow steps.</p>

      {status === 'done' ? (
        <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-lg p-6">
          <p className="text-emerald-400 font-medium mb-2">Skill created successfully</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input required className="w-full bg-white/5 border border-gray-700 rounded px-3 py-2 text-sm text-white placeholder-gray-600 focus:border-accent focus:outline-none" placeholder="What this skill produces" value={form.description || ''} onChange={(e) => set('description', e.target.value)} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input className="w-full bg-white/5 border border-gray-700 rounded px-3 py-2 text-sm text-white placeholder-gray-600 focus:border-accent focus:outline-none" placeholder="/skill-name" value={form.invocationCommand || ''} onChange={(e) => set('invocationCommand', e.target.value)} />
            </div>
            <div>
          ...
          <button type="submit" disabled={status === 'creating'} className="px-6 py-2.5 bg-accent text-white rounded font-medium text-sm hover:bg-accent/80 disabled:opacity-50">
            {status === 'creating' ? 'Creating...' : 'Create Skill'}
          </button>
